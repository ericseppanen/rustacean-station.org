...
Jake: Yeah, but no, I mean, we're doing, like, we're kind of making a game engine, but not really a game engine. It's-- it'll-- Yeah, we'll share more as we get more about that.
...
Ben: Talking about parallel stuff, like, how thoroughly parallel is an engine like Frostbite, like in terms of, like, is it like a few threads? Is it like 1000 threads? Is it--
...
Jake: We'll say that. Let's see. What's out favorite crate? Like, I mean, like, we use a lot of the classics, I guess, like `serde`, and--
...
Jake: Yeah. I mean, right now, like we're using Wasmer for our runtime and it's really quite good.
...
Ben: Great. Okay, that sounds fantastic. Do you think you'll anything to show any time soon? With regard to, like, an actual like, Pong clone that we can run?
...
Ben: It's good to have upgrade paths, where it's like, you know, you can do experiments with-- (???), some time, with some kind of thing, and then gradually it becomes more and more-- like goes from de-facto stable to de-jure stable, which is a pretty decent way of doing things, I think.

Yoshua: We're seeing a really fun one there, recently. Where we looked at (???), I guess someone pointed out to us that in the RFC for async/await there was a mention of the `IntoFuture` trait that would be accepted and we were like, oh, that's nice. Cool. Let's add it to `async-std`. Right? And then someone from `tokio` project, or-- I wrote a blog post about it, and someone from `tokio` project saw it, and was like, yeah, that's indeed really nice, and they put in the work to put it into, not just `futures-core`, but they just went straight to compiler and were like, hey, here's the thing, and here's a patch, and now it's, I think, either being merged or already merged.
...
Ben: I guess they-- is there-- is the async/await RFC still up to date with the kind of design, or is it gonna be like when they destabilized or made it its own feature, do they kind of say, hey, we're gonna rethink this.
...
Yoshua: Well, yeah, we have had to, for sure, but not as much as you might, like, think we had to.
...
Ben: How is documentation for `async-std`? If someone wanted to use it and learn it, oh, it doesn't look very good. (???)
...
Ben: Do you know of anyone, kind of, using `async-std` in a big (??) to, like, test, like you have to, kind of like, to pummel it for a while, run it through its paces?

Yoshua: Well, we've been doing that ourselves, internally, trying to be like, how do we stack up, right?

Ben: Doing what, though? Do you have, like, test suites, like big example applications, anyone using it in any kind of production capacity, like it's from Ferrous Systems, and so is there, like-- it's a company. Do they have any clients that they're using it for? Or--
...
Ben: And it is an exciting time right now.
...
Ben: Well, thanks, Joshua. Thanks for being on.
...
Ben: Yeah. So that's actually quite an old project, I think, originally. So `crossbeam` being, I think, was-- Aaron Turon made it many years ago. Back around Rust 1.0. And I remember reading blog posts about epoch-based garbage collection back then. At what point did you get involved?
...
Ben: So I think a different way of saying it is, we already have-- in Rust today we have `Arc`, `sync::Arc`, which is--
...
Ben: Okay, Interesting. And then hopefully, we'll see that someday if actually, like, are there any trade offs? Like, you say it's all better. Is it, like, more memory usage, or doesn't work as well on--
...
Ben: Wonderful. So very cool. What other parts of `crossbeam` are there, besides the channels. You mentioned some things before, I feel like. The scope threads, for example.
...
Ben: Oh, okay, yeah. I was trying to think of like, yeah, I usually say like "dee-queue," but that's definitely not how it's pronounced. There's no good way of pronouncing that word.
...
Stjepan: It's different. So `Once` is for initializing data once, whereas `AtomicCell` is like a cell, like the `Cell` type from the standard library. Except it's thread-safe, and you can update the contents of this that cell as many times as you want. So not just once.
...
Ben: Excellent. And there's a still MPMC?
...
Stjepan: So the underlying concurrent queue is the same as in `crossbeam`, but the, like-- instead of notifying threads when send or receive operations can proceed, we have to notify asyncronous tasks and use `Waker`s for that, so that part is different.

Ben: Okay, Cool. But other than that, it's all-- shares all the same code, benefits from all the same bug testing--
...
Ben: I would love to see a library like that reach 1.0. These foundational sort of libraries. Is there anyone-- if someone wanted to help with that, what would you, like, say, like, is there a chat channel they should see you on, or a forum?
